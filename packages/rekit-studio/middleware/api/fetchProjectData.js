  ...
  arr.forEach(item => {
    ...
    if (item.file) {
      ...
      item.hasTest =
        /^src\//.test(item.file) &&
        /\.js$/.test(item.file) &&
        fs.existsSync(path.join(utils.getProjectRoot(), testFile));
    }
    if (item.children) mapRelPathForDepsByArr(item.children);
    if (item.deps) {
      mapRelPathForDepsByArr([].concat(item.deps.components, item.deps.actions, item.deps.misc, item.deps.constants));
    }
  });
}
...
function fetchProjectData() {
  ...
  const syntaxErrors = {};
  Object.keys(rekitCore.vio.getFilesFailedToParse())
    .map(utils.getRelativePath)
    .forEach(k => {
      syntaxErrors[k] = true;
    });
  return {
