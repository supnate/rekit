...
const getFeatures = _.memoize(elementById => {
  return Object.values(elementById).filter(ele => ele.type === 'feature').map(f => f.name);
});

export class OverviewDiagram extends PureComponent {
  static propTypes = {
  ...
  constructor(props) {
    super(props);
    this.state.selectedFeatures = getFeatures(props.elementById);
  }
  ...
  componentDidMount() {
    ...
    this.tooltip = d3Tip()
      .attr('class', 'd3-tip')
      .offset([-10, 0])
      .html(d => this.props.elementById[d.id].name)
    ;
  ...
  drawGroups(groups, container) {
    const getCssClass = (d) => {
      ...
      if (d.type === 'file') {
        const ele = this.props.elementById[d.id];
  ...
  render() {
    const features = getFeatures(this.props.elementById);
    ...
    } else if (this.state.selectedFeatures.length < features.length) {
      featureSelectLabel = `${this.state.selectedFeatures.length}/${features.length} features`;
    }
    return (
